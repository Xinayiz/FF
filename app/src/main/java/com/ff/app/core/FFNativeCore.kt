...
class FFNativeCore : VpnCore() {
    ...
    class FFVpnService : VpnService() {
        private lateinit var engine: AiBypassEngine
        ...
        private fun handleSocksClient(client: Socket) {
            try {
                ...
                val remote = Socket("127.0.0.1", 1081)
                output.write(byteArrayOf(5, 0, 0, 1, 0, 0, 0, 0, 0, 0))
                executor.submit { pipe(input, remote.getOutputStream(), true) }
                executor.submit { pipe(remote.getInputStream(), output, false) }
            } catch (_: Exception) {} finally { client.close() }
        }

        private fun pipe(src: InputStream, dst: OutputStream, upstream: Boolean) {
            val buf = ByteArray(8192)
            try {
                var len: Int
                while (src.read(buf).also { len = it } != -1) {
                    val out = if (upstream) engine.applyBypass(buf.copyOf(len)) else buf.copyOf(len)
                    dst.write(out)
                    dst.flush()
                }
            } catch (_: Exception) {}
    ...
    override fun start(config: String) {
        running.set(true)
        val ctx = com.ff.app.FFApplication.appContext ?: return
        val intent = Intent(ctx, FFVpnService::class.java)
        ctx.startService(intent)
        vpnService = FFVpnService()
        vpnService?.startVpn(engine)
    }
    ...
    override fun stop() {
        ...
        vpnService?.stopVpn()
        val ctx = com.ff.app.FFApplication.appContext ?: return
        ctx.stopService(Intent(ctx, FFVpnService::class.java))
    }
